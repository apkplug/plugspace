...
//		});
	...
	private UserInfo getKF5UserInfo(){
		...
		if(data != null){
			UserHelper.userName = data.getString(UserHelper.USERNAME) == null?null:data.getString(UserHelper.USERNAME);
			UserHelper.userEmail = data.getString(UserHelper.USEREMAIL) == null?null:data.getString(UserHelper.USEREMAIL);
			UserHelper.userPhone = data.getString(UserHelper.USERPHONE) == null?null:data.getString(UserHelper.USERPHONE);
			UserHelper.appId = data.getString(UserHelper.APPID) == null?null:data.getString(UserHelper.APPID);
			UserHelper.helpAddress = data.getString(UserHelper.HELPADDRESS) == null?null:data.getString(UserHelper.HELPADDRESS);
		}

		kf5UserInfo.appId = UserHelper.appId;
		kf5UserInfo.helpAddress = UserHelper.helpAddress;
		kf5UserInfo.email = UserHelper.userEmail;
//		kf5UserInfo.deviceToken = "";
	...
	private void setKF5UI(){
		...
		activityUiConfig.topBarBackground = getResources().getColor(R.color.kf5_red);
